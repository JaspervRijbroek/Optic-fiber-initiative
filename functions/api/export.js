...
export async function onRequestGet({ request, env }) {
  ...
  try {
    const stmt = await env.DB
      .prepare('SELECT id, nombre, email, cru, unsubscribed_at, created_at FROM registrations ORDER BY created_at ASC')
      .all();
  ...
  // ── Build CSV ────────────────────────────────────────────────
  const header = ['ID', 'Nombre', 'Email', 'CRU', 'Baja', 'Fecha de Registro'].map(csvField).join(',');
  const rows = results.map((row) =>
    [row.id, row.nombre, row.email, row.cru, row.unsubscribed_at ?? '', row.created_at].map(csvField).join(',')
  );
